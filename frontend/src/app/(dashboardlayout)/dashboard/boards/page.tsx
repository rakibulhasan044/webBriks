    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] p-6 md:p-8 bg-slate-50/50 -m-6 md:-m-8">
      ...
      {boards.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/60 backdrop-blur-sm p-12 text-center">
          ...
          <p className="text-slate-500 max-w-sm mb-8 text-[15px] leading-relaxed">
            You haven&apos;t created any boards yet, or no boards match your
            search.
          </p>
          <CreateBoardModal>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6">
          {boards.map((board: any, index: number) => {
            ...
            const themes = [
              {
                ...
                text: "text-amber-700",
                ring: "ring-amber-200",
                hoverBorder: "group-hover:border-amber-200/60",
              },
              {
                ...
                hoverBorder: "group-hover:border-emerald-200/60",
              },
              {
                ...
                text: "text-sky-700",
                ring: "ring-sky-200",
                hoverBorder: "group-hover:border-sky-200/60",
              },
              {
                gradient: "from-violet-400/90 to-purple-500/90",
                accent: "bg-violet-500",
                light: "bg-violet-50",
                text: "text-violet-700",
                ring: "ring-violet-200",
                hoverBorder: "group-hover:border-violet-200/60",
              },
              {
                ...
                text: "text-rose-700",
                ring: "ring-rose-200",
                hoverBorder: "group-hover:border-rose-200/60",
              },
              {
                gradient: "from-cyan-400/90 to-indigo-500/90",
                accent: "bg-cyan-500",
                light: "bg-cyan-50",
                text: "text-cyan-700",
                ring: "ring-cyan-200",
                hoverBorder: "group-hover:border-cyan-200/60",
              },
            ];
            ...
            return (
              <div
                key={board.id}
                className={`relative group flex flex-col bg-slate-50 rounded-3xl border border-slate-100 shadow-[0_2px_16px_-4px_rgba(0,0,0,0.06)] hover:shadow-[0_12px_40px_-8px_rgba(0,0,0,0.15)] hover:-translate-y-1 transition-all duration-300 min-h-[300px] overflow-hidden ${theme.hoverBorder}`}
              >
                {/* Top accent line */}
                ...
                >
                  ...
                  {board.coverImage ? (
                    ...
                    <div
                      className={`h-36 w-full bg-gradient-to-br ${theme.gradient} flex items-center justify-center text-white/40 flex-shrink-0 relative overflow-hidden`}
                    >
                      <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_30%,rgba(255,255,255,0.2),transparent)]" />
                      <span className="font-bold text-5xl relative z-10 drop-shadow-sm">
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    <div className="mt-auto flex items-center justify-end pt-5">
                      <div className="flex items-center -space-x-2">
                        <Avatar
                          className={`h-7 w-7 border-2 border-white shadow-sm ring-1 ${theme.ring}`}
                        >
                          {board.owner?.photo && (
                            <AvatarImage
                          ...
                          <AvatarFallback
                            className={`text-[10px] ${theme.light} ${theme.text} font-bold`}
                          >
                        ...
                        {board.members
                          ...
                          .map((member: any, mIdx: number) => {
                            ...
                            return (
                              <Avatar
                                key={member.id}
                                className={`h-7 w-7 border-2 border-white shadow-sm ring-1 ${mt.ring}`}
                              >
                                {member.user?.photo && (
                                  <AvatarImage
                                    src={
                                      getImageUrl(member.user.photo) as string
                                    }
                                  />
                                )}
                                <AvatarFallback
                                  className={`text-[10px] ${mt.light} ${mt.text} font-bold`}
                                >
