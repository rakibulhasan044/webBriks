        ...
        <DropdownMenu>
          <DropdownMenuTrigger className="flex items-center gap-3 focus:outline-none rounded-lg p-1.5 hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-200">
            ...
            <div className="hidden sm:flex flex-col items-start text-left pr-2">
              ...
              <span className="text-xs text-slate-500 leading-tight">{user?.email || "No email"}</span>
            </div>
          </DropdownMenuTrigger>
              ...
              <DropdownMenuLabel className="font-normal p-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col space-y-1 overflow-hidden">
                    <p className="text-sm font-semibold text-slate-900 leading-none truncate">{user?.name || "User"}</p>
                    <p className="text-xs text-slate-500 leading-none truncate">{user?.email || "No email"}</p>
                  </div>
                </div>
              </DropdownMenuLabel>
            </DropdownMenuGroup>
