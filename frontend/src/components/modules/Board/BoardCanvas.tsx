...
export default function BoardCanvas({
  ...
  React.useEffect(() => {
        ...
        prev.map((col: IColumn) => {
          ...
          if (col.id === updatedTask.columnId) {
            ...
            if (!hasTask) {
              ...
              newTasks = (col.tasks || []).map((t: ITask) =>
                t.id === updatedTask.id
                  ? {
                      ...t,
            ...
            return {
              ...col,
              tasks: newTasks.sort(
                (a: ITask, b: ITask) => Number(a.position) - Number(b.position),
              ),
            };
          }
    ...
    socket.on("task_deleted", (data: { id: string }) => {
      setColumns((prev) =>
        prev.map((col: IColumn) => ({
          ...col,
          tasks: (col.tasks || []).filter((t: ITask) => t.id !== data.id),
        })),
      );
    });
  ...
  const handleDeleteTask = async (taskId: string) => {
    ...
    setColumns((prev) =>
      prev.map((col: IColumn) => ({
        ...col,
        tasks: (col.tasks || []).filter((t: ITask) => t.id !== taskId),
      })),
    );
    ...
    const handleEvent = (e: any) => {
      ...
      const tempTask = {
        ...
        assignees: payload.assigneeIds
          ? payload.assigneeIds
              .map((id: string) => members.find((m: IUser) => m.id === id))
              .filter(Boolean)
          : [],
      };
      ...
      setColumns((prev) =>
        prev.map((col: IColumn) => {
          if (col.id === columnId) {
            return { ...col, tasks: [...(col.tasks || []), tempTask as ITask] };
          }
          return col;
        }),
      );
  ...
  const handleOptimisticCreate = (columnId: string, payload: any) => {
    ...
    setColumns((prev) =>
      prev.map((col: IColumn) => {
        if (col.id === columnId) {
          return { ...col, tasks: [...(col.tasks || []), tempTask as ITask] }; // Append to bottom as new tasks usually go to bottom, or prepend
        }
        return col;
      }),
    );
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    const sourceColIndex = columns.findIndex(
      (col: IColumn) => col.id === source.droppableId,
    );
    const destColIndex = columns.findIndex(
      (col: IColumn) => col.id === destination.droppableId,
    );
