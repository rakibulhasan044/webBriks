...
export class BoardMemberService {
  ...
  async addMember(boardId: string, currentUserId: string, dto: AddMemberDto) {
    ...
    if (board.ownerId !== currentUserId) throw new ForbiddenException('Only the board owner can add members');
    ...
    if (userToAdd.id === board.ownerId) throw new ConflictException('User is the owner of this board');

    // Check if already a member
    const existingMember = await this.prisma.boardMember.findUnique({
      where: { boardId_userId: { boardId, userId: userToAdd.id } }
    });
    if (existingMember) throw new ConflictException('User is already a member of this board');
  ...
  async getMembers(boardId: string, currentUserId: string) {
    ...
    const hasAccess = board.ownerId === currentUserId || board.members.some(m => m.userId === currentUserId);
  ...
  async removeMember(boardId: string, userIdToRemove: string, currentUserId: string) {
    const board = await this.prisma.board.findUnique({ where: { id: boardId } });
    if (!board) throw new NotFoundException('Board not found');
